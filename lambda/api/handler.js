  ...
  try {
    ...
    if (path === '/api/ideas' && method === 'POST') {
      ...
      const systemPrompt = hasExisting
        ? `You are a coloring book planner. The book already has ${existingPages.length} pages. Generate ${sceneCount} NEW additional pages that complement the existing ones without duplicating them. Respond in JSON with key: pages (array of {title, scene, prompt, caption}). "scene" is a short 1-sentence description. "prompt" is a detailed ART DIRECTION prompt for an image generation AI — describe what to draw: subjects, poses, expressions, spatial composition, environment details. Do NOT write coloring instructions in the prompt. "caption" is a fun activity instruction. Match visual complexity to the target audience age.`
        : 'You are a coloring book planner. Propose a book concept and page scenes. Respond in JSON with keys: title, tagLine, concept, pages (array of {title, scene, prompt, caption}). "scene" is a short 1-sentence description of the page for display. "prompt" is a detailed ART DIRECTION prompt for an image generation AI — describe what to draw: subjects, poses, expressions, spatial composition, environment details. Do NOT write coloring instructions or activity text in the prompt field. "caption" is a short fun activity instruction printed below the image (e.g. "Color the dragon and count its scales!"). Match visual complexity to the target audience age.';
    ...
    if (path === '/api/ideas/page' && method === 'POST') {
      ...
      const systemPrompt = `You are a coloring book planner. Given the book context below, regenerate ONLY page ${Number(pageIndex) + 1}. Return JSON: {"title": "...", "scene": "...", "prompt": "...", "caption": "..."}. "scene" is a short 1-sentence description for display. "prompt" is a detailed ART DIRECTION prompt for an image generation AI — describe what to draw: subjects, poses, expressions, spatial composition, environment details. Do NOT write coloring instructions in the prompt. "caption" is a short fun activity instruction printed below the image. Match visual complexity to the target audience age.`;
      const userContent = [
